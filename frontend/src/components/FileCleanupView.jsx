...
export function FileCleanupView() {
  ...
  const groups = useMemo(() => cleanupReport.folder_media_duplicates || cleanupReport.groups || [], [cleanupReport]);
  ...
  useEffect(() => {
    const validGroupIds = new Set(filteredGroups.map((group) => String(group.id)));
    setSelectedGroupIds((current) => current.filter((id) => validGroupIds.has(String(id))));
  }, [filteredGroups]);

  const selectedGroups = useMemo(
    () => filteredGroups.filter((group) => selectedGroupIds.includes(String(group.id))),
    ...
    <Flex vertical gap={16}>
      <Card
        title={
          <Space>
            <FileSearchOutlined />
            <span>Provider Duplicate Groups</span>
          </Space>
        }
        ...
        <Flex vertical gap={16}>
          ...
          <Descriptions
            ...
            items={[
              ...
              { key: "groups", label: "Groups", children: groups.length },
          ...
          {!groups.length ? (
            <Alert
              ...
              description="No movie or episode folder currently contains multiple candidate video files in the reachable provider library."
            />
          ) : null}
          ...
          {errors.length ? (
            <Alert
              ...
              description={errors.map((item) => `${item.provider}: ${item.message}`).join(" • ")}
            />
          ) : null}
          ...
          <Table
            ...
            locale={{ emptyText: <Empty description="No provider duplicate groups yet. Run a cleanup scan first." /> }}

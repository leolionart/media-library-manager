...
`cleanup-scan` hiện bao gồm cả provider duplicate cleanup và empty duplicate folder cleanup.
...
Khi chạy bằng Vite dev server, các request `/api/*` được proxy mặc định sang backend local:

```text
http://127.0.0.1:8766
```

Nếu backend chạy ở host hoặc port khác, có thể override trước khi chạy frontend:
